...
function init(includeList, excludeList) {
    let include = [], exclude = [];
    if (includeList) {
...
function showFloatingIcon() {
    const prototyperScript = document.querySelector('script[src*="prototyper.js"]');
    const floatingIcon = document.createElement('div');
    floatingIcon.classList.add('floating-icon');
    floatingIcon.textContent = 'P';
    floatingIcon.onclick = () => {
        floatingIcon.remove();
        init(prototyperScript.dataset.include, prototyperScript.dataset.exclude);
    }
